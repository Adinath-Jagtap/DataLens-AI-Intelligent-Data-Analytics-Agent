...
export default function Signup() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.success("Account created! Welcome to DataLens AI.");
      navigate("/home", { replace: true });
    } catch (err) {
      toast.error(err.message?.replace("Firebase: ", "").replace(/\(.*\)/, "").trim() || "Sign up failed");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-surface-0 flex items-center justify-center px-4 py-12">
      <div className="fixed top-1/3 left-1/2 -translate-x-1/2 w-[600px] h-[400px] pointer-events-none rounded-full"
           style={{ background: "radial-gradient(ellipse, rgba(167,139,250,0.07) 0%, transparent 70%)" }} />
      ...
      <div className="relative w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <p className="text-slate-400 text-sm mt-1 font-body">Free forever. No credit card needed.</p>
        </div>
        ...
        <div className="glass rounded-3xl p-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <div className="relative">
                ...
                <input type="text" value={name} onChange={e => setName(e.target.value)}
                       className="input-field pl-10" placeholder="Adinath Jagtap" required />
              </div>
            </div>
            <div>
            ...
            <button type="submit" disabled={loading} className="btn-primary w-full justify-center py-3 mt-2">
              {loading ? <span className="w-4 h-4 border-2 border-surface-0 border-t-transparent rounded-full animate-spin" /> : "Create account"}
            </button>
          </form>
